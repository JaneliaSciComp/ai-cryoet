...
`target_metadata` is `cryoet_catalog.orm.Base.metadata` so autogenerate sees
...
`cryoet_catalog.db.DEFAULT_DB_URL`. The `sqlalchemy.url` value in
...
from cryoet_catalog.db import DEFAULT_DB_URL  # noqa: E402
...
def _resolve_url() -> str:
    ...
    4. `cryoet_catalog.db.DEFAULT_DB_URL`.
    """
...
def run_migrations_online() -> None:
    ...
    Programmatic callers (e.g. ``cryoet_catalog.db.init_schema``) may pass
