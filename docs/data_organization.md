...
2. A **raw tilt-series quality score** (`raw_tilt_series_quality`): an integer on a 1–5 rubric, the author's estimate of the RAW tilt series (alignability + projection-image survival) — **5** Excellent (reconstructions could be publication-ready), **4** Good (useful for analysis such as subtomogram averaging or segmentation), **3** Medium (minor projection images discarded before reconstruction), **2** Marginal (major projection images discarded; usable only after heavy manual work), **1** Low (not alignable / not useful for analysis). (This replaces the former free-text `quality` field, which has been removed.)
